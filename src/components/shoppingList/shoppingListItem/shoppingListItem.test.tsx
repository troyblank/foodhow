...
describe('ShoppingListItem', () => {
	...
	it('Should render list item with a label for the checkbox.', () => {
		const item = mockShoppingListItem()
	...
	it('Should call onToggle when Space key is pressed on checkbox.', async () => {
		const user = userEvent.setup()
		const mockOnToggle = jest.fn()
		const itemId = chance.natural()
		const item = mockShoppingListItem({ id: itemId })
	...
	it('Should prevent default behavior when Space key is pressed on checkbox.', async () => {
		const user = userEvent.setup()
		const mockOnToggle = jest.fn()
		const item = mockShoppingListItem()
		const { getByRole } = render(<ShoppingListItem item={item} checked={false} onToggle={mockOnToggle} />)
	...
	it('Should not call onToggle when other keys are pressed on checkbox.', async () => {
		const user = userEvent.setup()
		const mockOnToggle = jest.fn()
		const item = mockShoppingListItem()
		const { getByRole } = render(<ShoppingListItem item={item} checked={false} onToggle={mockOnToggle} />)
