...
describe('Use Auth', () => {
	...
	it('should allow attempts to sign in', async () => {
		...
		await act(() => attemptToSignIn({ username, password }))
	...
	it('should handle any errors with attempts to sign in', async () => {
		...
		await expect(attemptToSignIn({ username, password })).rejects.toThrow(error)
	})
	...
	it('should handle any errors with any nextStep not accounted for', async () => {
		...
		await expect(attemptToSignIn({ username, password })).rejects.toThrow(`UI flow is missing a step: ${badNextStep}. This is not your fault please contact support.`)
	})

	it('should use default error message when signed in is false and step is DONE', async () => {
		const username: string = chance.name()
		const password: string = chance.word()
		...
		await expect(attemptToSignIn({ username, password })).rejects.toThrow('Something went wrong.')
	})
})
