...
describe('Button', () => {
	...
	it('should not call the button handler when disabled and clicked', async () => {
		const text = chance.word()
		const buttonClickHand = jest.fn()

		const { getByRole } = render(<Button text={text} buttonClickHand={buttonClickHand} disabled={true} />)

		await userEvent.click(getByRole('button'))

		expect(buttonClickHand).not.toHaveBeenCalled()
	})

	it('should not call the button handler when isPending and clicked', async () => {
		const text = chance.word()
		const buttonClickHand = jest.fn()

		const { getByRole } = render(<Button text={text} buttonClickHand={buttonClickHand} isPending={true} />)
